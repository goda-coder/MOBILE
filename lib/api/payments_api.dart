...
class PaymentsApi {
  ...
  Future<PaymentIntentStatusResponse> paymentIntentStatus(String paymentIntentId) async {
    try {
      final r = await _c.dio.get('/api/v1/payments/status/$paymentIntentId');
      return PaymentIntentStatusResponse.fromJson(r.data as Map<String, dynamic>);
    } on DioException catch (e) { throw ApiClient.toApiError(e); }
  }
}
...
class AdminApi {
  ...
  Future<List<PendingKycSummary>> pendingKyc({int skip = 0, int take = 25}) async {
    try {
  ...
  Future<void> approve(String id, String reason) async {
    try {
      await _c.dio.post('/api/v1/admin/kyc/$id/approve', data: {'reason': reason});
    } on DioException catch (e) { throw ApiClient.toApiError(e); }
  }

  Future<void> reject(String id, String reason) async {
    try {
      await _c.dio.post('/api/v1/admin/kyc/$id/reject', data: {'reason': reason});
    } on DioException catch (e) { throw ApiClient.toApiError(e); }
  }
}
